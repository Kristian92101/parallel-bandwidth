...
#define TITLE_MEMORY_GRAPH "Results from Parallel bandwidth " RELEASE
...
static enum {
	OUTPUT_MODE_GRAPH=1,
	OUTPUT_MODE_CSV=2,
	OUTPUT_MODE_MPI_SPLIT=4,
	OUTPUT_MODE_OMP_SPLIT=8,
} outputMode;

static FILE **csv_output_files = NULL;
...
static SimpleGraphing **graphs = NULL;
...
static int chunk_sizes[] = {
	...
	96 * 1024 * 1024,
	128 * 1024 * 1024,
#if defined(TEST_L4) || defined(TEST_DRAM)
	160 * 1024 * 1024,
	192 * 1024 * 1024,
	224 * 1024 * 1024,
	256 * 1024 * 1024,
...
void dataBegins (const char *title, const char *subtitle)
{
	...
	if ((outputMode & OUTPUT_MODE_MPI_SPLIT) || ((comm_rank == 0)))
	{
		for(int tid = 0; tid < omp_get_max_threads(); ++tid)
		{
			snprintf(curr_csv_file_path, sizeof(curr_csv_file_path)-1, "%s", csv_file_path);

			if ((outputMode & OUTPUT_MODE_OMP_SPLIT) || (tid == 0))
			{
				if (outputMode & OUTPUT_MODE_GRAPH) {
					...
					#pragma omp critical
					{
						graphs[tid] = SimpleGraphing_newWithSize (GRAPH_WIDTH, GRAPH_HEIGHT);
						retain (graphs[tid]);
					}

					$(graphs[tid], setXAxisMode, MODE_X_AXIS_LOG2);
					if (title) {
						$(graphs[tid], setTitle, String_newWithCString(title));
					}
					if (subtitle) {
						$(graphs[tid], setSubtitle, String_newWithCString(subtitle));
					}
				}	

				if (outputMode & OUTPUT_MODE_CSV) {
					if (csv_output_files[tid])
						error (__FUNCTION__, "CSV file already initialized.");
					if (outputMode & OUTPUT_MODE_MPI_SPLIT)
					{
						char rank_string[32];
						snprintf(rank_string, sizeof(rank_string)-1, "_rank_%d", comm_rank);
						strcat(curr_csv_file_path, rank_string);
					}
					if (outputMode & OUTPUT_MODE_OMP_SPLIT)
					{
						char tid_string[32];
						snprintf(tid_string, sizeof(tid_string)-1, "_thread_%d", tid);
						strcat(curr_csv_file_path, tid_string);
					}
					strcat(curr_csv_file_path, ".csv");
					csv_output_files[tid] = fopen (curr_csv_file_path, "wb");
					if (!csv_output_files[tid]) {
						error (__FUNCTION__, "Cannot open CSV output file.");
					}
					if (title)
						fprintf (csv_output_files[tid], "%s\n", title);
					unsigned int * cpus = malloc(omp_get_max_threads()*sizeof(unsigned int));
					#pragma omp parallel default(none) shared(cpus)
					{
                        cpus[omp_get_thread_num()] = sched_getcpu();
					}
					fprintf (csv_output_files[tid], "Core: %d, MPI:(%d in 0-%d), OMP:(%d in 0-%d)\n", cpus[tid], comm_rank, comm_size-1, tid, omp_get_max_threads()-1);
					fprintf (csv_output_files[tid], "Mode: MPI: %s, OMP: %s\n", (outputMode & OUTPUT_MODE_MPI_SPLIT) ? "split" : "aggregated", (outputMode & OUTPUT_MODE_OMP_SPLIT) ? "split" : "aggregated");
					// fprintf (csv_output_files[tid], "Size, Thrd/Proc, OMP Agg., OMP Avg., OMP Max., OMP Min., MPI Agg., MPI Avg., MPI Max., MPI Min.\n");
                    free(cpus);
				}
			}
		}
		...
		for(int tid = 0; tid < omp_get_max_threads(); ++tid)
		{
			if ((outputMode & OUTPUT_MODE_OMP_SPLIT) || (tid == 0))
			{
				if (outputMode & OUTPUT_MODE_GRAPH) {
					...
					$(graphs[tid], addLine, name, parameter);
				}
		...
		for(int tid = 0; tid < omp_get_max_threads(); ++tid)
		{
			if ((outputMode & OUTPUT_MODE_OMP_SPLIT) || (tid == 0))
			{
				if (outputMode & OUTPUT_MODE_GRAPH) {
					...
					$(graphs[tid], make);
					MutableImage *image = NULL;
					#pragma omp critical
					{
						image = $(graphs[tid], image);
					}
					char graph_name[512];
					if (outputMode & OUTPUT_MODE_OMP_SPLIT)
					{
						snprintf(graph_name, sizeof(graph_name)-1, "%s_rank_%d_thred_%d.%s", parameter, comm_rank, tid, "bmp");
					}
					else
					{
						if (outputMode & OUTPUT_MODE_MPI_SPLIT)
						{
							snprintf(graph_name, sizeof(graph_name)-1, "%s_rank_%d.%s", parameter, comm_rank, "bmp");
						}
						else
						{
							snprintf(graph_name, sizeof(graph_name)-1, "%s.%s", parameter, "bmp");
						}
					}
					#pragma omp critical
					{
						$(image, writeBMP, graph_name);
					}
...
void 
dataAddDatum (long x, long y, long * results)
{
	require(outputMode) else { error (__FUNCTION__, "Bad output mode."); }
	//==========
	...
	double omp_avg;
	double omp_agg = 0;
	double omp_min = results[0];
	double omp_max = results[0];
	...
	{
		omp_agg += results[tid];
		if (results[tid] > omp_max)
		{
			omp_max = results[tid];
		}

		if (results[tid] < omp_min)
		{
			omp_min = results[tid];
		}
	}

	omp_avg = omp_agg / (double)threads;
	...
	MPI_Reduce(&omp_agg, &mpi_agg, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(&omp_agg, &mpi_max, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
	MPI_Reduce(&omp_agg, &mpi_min, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
	
	mpi_avg = mpi_agg / (double)comm_size;
		...
		if (outputMode & OUTPUT_MODE_MPI_SPLIT)
		{
			if (outputMode & OUTPUT_MODE_OMP_SPLIT)
			{
				for(int tid = 0; tid < omp_get_max_threads(); ++tid)
				{
					...
					$(graphs[tid], addPoint, x, results[tid]);
				}
			}
			else
			{
	...
	if (outputMode & OUTPUT_MODE_CSV) 
	{
		...
		if (outputMode & OUTPUT_MODE_OMP_SPLIT)
		{
			for(int tid = 1; tid < omp_get_max_threads(); ++tid)
			{
				...
				fprintf (csv_output_files[tid], "%lld, %.2f\n", (long long)x, (double)results[tid]/10.);
				fflush (csv_output_files[tid]);
			}
		}

		if ((outputMode & OUTPUT_MODE_MPI_SPLIT) && (comm_rank != 0))
		{
			if (!csv_output_files[0]) 
					error (__FUNCTION__, "CSV output not initialized.");

				fprintf (csv_output_files[0], "%lld, %.2f, %.2f, %.2f, %.2f, %.2f \n", (long long)x, (double)results[0]/10., (double)omp_agg/10., (double)omp_avg/10., (double)omp_max/10.,(double)omp_min/10.);
				fflush (csv_output_files[0]);
		}
		...
		if (comm_rank == 0)
		{
			...
			fprintf (csv_output_files[0], "%lld, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f \n", 
				(long long)x, 
				(double)results[0]/10., 
				(double)omp_agg/10., 
				(double)omp_avg/10., 
...
main (int argc, char **argv)
{
	...
	for (size_t i = 0; chunk_sizes[i] != 0; i++)
	{
		chunk_sizes[i] *= threads;
	}
	

	csv_output_files = malloc(threads * sizeof(FILE *));
	graphs           = malloc(threads * sizeof(SimpleGraphing *));
	memset(csv_output_files, 0, threads * sizeof(FILE *));
	memset(graphs          , 0, threads * sizeof(SimpleGraphing *));

	long *results = malloc(threads * sizeof(long));
	...
	while (i < argc) {
		...
		if (!strcmp ("--split-level", s) && i != argc) {
			int split_level = atoi(argv[i++]);
			if (split_level > 0)
			{
				outputMode |= OUTPUT_MODE_MPI_SPLIT;
			}
			if (split_level > 1)
			{
				outputMode |= OUTPUT_MODE_OMP_SPLIT;
			}
		}
		else
		if (!strcmp ("--csv", s) && i != argc) {
			...
			if (i < argc)
				snprintf (csv_file_path, sizeof(csv_file_path)-1, "%s", argv[i++]);
			else
				usage ();
		}
		else
	...
	$(console, println, "This is Parallel bandwidth version " RELEASE);
	$(console, println, "Copyright (C) 2005-2021 by Zack T Smith. (original author)");
    $(console, println, "Copyright (C) 2023 by Kristian Kadlubiak.");
	$(console, newline);
	...
	i = 0;
	memset(results, 0, threads*sizeof(long));
	while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
		MPI_Barrier(MPI_COMM_WORLD);
		long amount = $(routines, read, results, chunk_size, NO_SSE2, false, false);

		dataAddDatum (chunk_size, amount, results);
	}
	...
	while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
		MPI_Barrier(MPI_COMM_WORLD);
		long amount = $(routines, read, results, chunk_size, NEON_128BIT, false, false);
	...
	if (use_sse2) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			MPI_Barrier(MPI_COMM_WORLD);
			long amount = $(routines, read, results, chunk_size, SSE2, false, false);
	...
	if (cpu->has_avx) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, read, results, chunk_size, AVX, false, false);
	...
	if (!cpu->is_amd && cpu->has_avx512_f) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, read, results, chunk_size, AVX512, false, false);
	...
	if (use_sse4) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			MPI_Barrier(MPI_COMM_WORLD);
			long amount = $(routines, read, results, chunk_size, SSE2_BYPASS, false, false);
...
// SEQ WRITES
	...
	while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
		if (!(chunk_size & 128)) {
			MPI_Barrier(MPI_COMM_WORLD);
			long amount = $(routines, write, results, chunk_size, NO_SSE2, false, false);
	...
	while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
		MPI_Barrier(MPI_COMM_WORLD);
		long amount = $(routines, write, results, chunk_size, NEON_128BIT, false, false);
	...
	if (use_sse2) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			MPI_Barrier(MPI_COMM_WORLD);
			long amount = $(routines, write, results, chunk_size, SSE2, false, false);
	...
	if (cpu->has_avx) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, write, results, chunk_size, AVX, false, false);
	...
	if (!cpu->is_amd && cpu->has_avx512_f) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, write, results, chunk_size, AVX512, false, false);
	...
	if (use_sse4) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			MPI_Barrier(MPI_COMM_WORLD);
			long amount = $(routines, write, results, chunk_size, SSE2_BYPASS, false, false);
	...
	if (cpu->has_avx) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, write, results, chunk_size, AVX_BYPASS, false, false);
...
//RANDOM READS
	...
	while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
		if (!(chunk_size & 128)) {
			MPI_Barrier(MPI_COMM_WORLD);
			long amount = $(routines, read, results, chunk_size, NO_SSE2, true, false);
	...
	if (use_sse2) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, read, results, chunk_size, SSE2, true, false);
	...
	if (cpu->has_avx) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, read, results, chunk_size, AVX, true, false);
	...
	while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
		MPI_Barrier(MPI_COMM_WORLD);
		long amount = $(routines, read, results, chunk_size, NEON_128BIT, true, false);
	...
	if (use_sse4) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, read, results, chunk_size, SSE2_BYPASS, true, false);
...
//RANDOM WRITES
	...
	while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
		if (!(chunk_size & 128)) {
			MPI_Barrier(MPI_COMM_WORLD);
			long amount = $(routines, write, results, chunk_size, NO_SSE2, true, false);
	...
	while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
		MPI_Barrier(MPI_COMM_WORLD);
		long amount = $(routines, write, results, chunk_size, NEON_128BIT, true, false);
	...
	if (use_sse2) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, write, results, chunk_size, SSE2, true, false);
	...
	if (cpu->has_avx) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, write, results, chunk_size, AVX, true, false);
	...
	if (use_sse4) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, write, results, chunk_size, SSE2_BYPASS, true, false);
	...
	if (use_sse2) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			MPI_Barrier(MPI_COMM_WORLD);
			long amount = $(routines, copy, results, chunk_size, SSE2, false);
	...
	if (cpu->has_avx) {
		...
		while ((chunk_size = chunk_sizes [i++]) && chunk_size <= chunk_limit) {
			if (!(chunk_size & 128)) {
				MPI_Barrier(MPI_COMM_WORLD);
				long amount = $(routines, copy, results, chunk_size, AVX, false);
