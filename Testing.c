...
//----------------------------------------------------------------------------
static int Testing_calculate_result (Testing* self, unsigned long chunk_size, long long total_loops, uint64_t diff, bool print)
{
	if (!diff) {
...
//----------------------------------------------------------------------------
long Testing_write (Testing *self, long *results, unsigned long size, TestingMode mode, bool random, bool first_touch)
{
	unsigned long *chunk;
	...
	if (first_touch)
	{
		#pragma omp parallel default(none) firstprivate (chunk, size, grain)
		{
			unsigned long threads = omp_get_num_threads();
			unsigned long tid     = omp_get_thread_num();

			// Setting poiter to memory correctly on per-thread basis 
			unsigned long thread_grains   = size / (threads*grain);
			unsigned long reminder_grains = (size - (thread_grains*threads*grain)) / grain;

			if(tid < reminder_grains)
			{
				thread_grains++;
			}

			unsigned long size_per_thread = thread_grains*grain;

			if(tid < reminder_grains)
			{
				chunk += (size_per_thread*tid) / sizeof(unsigned long);
			}
			else
			{
				chunk += (reminder_grains*grain + size_per_thread*tid) / sizeof(unsigned long);
			}
			memset (chunk, 0, size_per_thread);
		}
	}
	else
	{
	...
	if (random) {
		...
		if (first_touch)
		{
			#pragma omp parallel default(none) firstprivate (chunk_ptrs, chunk, size, grain)
			{
				unsigned long threads = omp_get_num_threads();
				unsigned long tid     = omp_get_thread_num();

				// Setting poiter to memory pointers array correctly on per-thread basis 
				unsigned long p_size = size/256;

				unsigned long thread_p_size   = p_size / (threads);
				unsigned long reminder_p_size = (p_size - (thread_p_size*threads));

				if(tid < reminder_p_size)
				{
					thread_p_size++;
				}

				unsigned long p_size_per_thread = thread_p_size;
				unsigned long offset;

				if(tid < reminder_p_size)
				{
					offset = p_size_per_thread*tid;
				}
				else
				{
					offset = reminder_p_size + p_size_per_thread*tid;
				}

				for (int i = 0; i < p_size_per_thread; i++) {
					chunk_ptrs [offset + i] = (unsigned long*) (((char*)chunk) + 256 * (offset + i));
				}
			}
		}
		else
...
#pragma omp parallel default(none) shared(self, results, mode, random) firstprivate (value, chunk_ptrs, chunk, size, grain, usec_per_test) reduction(+:diff, total_count)
	{
		...
		if(tid < reminder_p_size)
		{
			chunk_ptrs += p_size_per_thread*tid;
		}
		else
		{
			chunk_ptrs += reminder_p_size + p_size_per_thread*tid;
		}

		usec_per_test = ((usec_per_test - 1) / threads) + 1;
		...
		while ((p_size_per_thread != 0) && (size_per_thread != 0) && (diff < usec_per_test)) {
			total_count += loops;
			...
			switch (mode) {
				...
				break;
					case NEON_128BIT:
				if (random)
					RandomWriterVector (chunk_ptrs, p_size_per_thread, loops, value);
				else
								WriterVector (chunk, size_per_thread, loops, value);
							break;
	#endif
			...
			case SSE2:
				if (random)
					RandomWriterSSE2 (chunk_ptrs, p_size_per_thread, loops, value);
				else {
					if (size & 128)
						WriterSSE2_128bytes (chunk, size_per_thread, loops, value);
					else
						WriterSSE2 (chunk, size_per_thread, loops, value);
				}
				break;

			case SSE2_BYPASS:
				if (random)
					RandomWriterSSE2_bypass (chunk_ptrs, p_size_per_thread, loops, value);
				else {
					if (size & 128)
						WriterSSE2_128bytes_bypass (chunk, size_per_thread, loops, value);
					else
						WriterSSE2_bypass (chunk, size_per_thread, loops, value);
				}
				break;

			case AVX:
				if (!random) {
					WriterAVX (chunk, size_per_thread, loops, value);
				} else {
	#ifdef IS_64BIT
					RandomWriterAVX (chunk_ptrs, p_size_per_thread, loops, value);
	#endif
				}
				break;
			...
			case AVX512:
				if (!random) {
	#ifdef IS_64BIT
					WriterAVX512 (chunk, size_per_thread, loops, value);
	#endif
				}
				break;
			...
			case AVX_BYPASS:
				if (!random) {
					WriterAVX_bypass (chunk, size_per_thread, loops, value);
				}
				break;
	#endif
			...
			default:
				if (random)
					RandomWriter (chunk_ptrs, p_size_per_thread, loops, value);
				else {
	#ifdef x86
					if (size & 128)
						Writer_128bytes (chunk, size_per_thread, loops, value);
					else
	#endif
						Writer (chunk, size_per_thread, loops, value);
				}
			}
			...
			if (random)
			{
				results[tid] = $(self, calculate_result, p_size_per_thread*256, total_count, diff, false);
			}
			else
			{
				results[tid] = $(self, calculate_result, size_per_thread, total_count, diff, false);
			}
		}
	}
	...
	int result = $(self, calculate_result, size, total_count, diff, true);
	$(console, flush);
...
//----------------------------------------------------------------------------
long Testing_read (Testing *self, long *results, unsigned long size, TestingMode mode, bool random, bool first_touch)
{
	unsigned long diff = 0;
	unsigned long long total_count = 0;
	unsigned long *chunk;
	...
	if (random) {
		...
		if (first_touch)
		{
			#pragma omp parallel default(none) firstprivate (chunk, chunk_ptrs, size, grain)
			{
				unsigned long threads = omp_get_num_threads();
				unsigned long tid     = omp_get_thread_num();
	...
	#pragma omp parallel default(none) shared(self, results, mode, random) firstprivate (chunk_ptrs, chunk, size, grain, usec_per_test) reduction(+:diff, total_count)
	{
		unsigned long long loops;
		...
		// printf("\nThread %lu: size in bytes: %lu,  size per thread in bytes %ld, starting address %ld\n", tid, size, size_per_thread, (unsigned long)chunk);

		// printf("\nThread %lu: size of pointers: %lu,  size of pointers per thread %ld, starting address %ld\n", tid, p_size, p_size_per_thread, (unsigned long)chunk_ptrs);
		...
		while ((p_size_per_thread != 0) && (size_per_thread != 0) && (diff < usec_per_test)) {
			...
			case SSE2:
				if (random)
					RandomReaderSSE2 (chunk_ptrs, p_size_per_thread, loops);
				else {
					if (size & 128)
						ReaderSSE2_128bytes (chunk, size_per_thread, loops);
					else
						ReaderSSE2 (chunk, size_per_thread, loops);
				}
				break;
			...
			case SSE2_BYPASS:
				...
				//
				if (random)
					RandomReaderSSE2_bypass (chunk_ptrs, p_size_per_thread, loops);
				else {
					if (size & 128)
						ReaderSSE2_128bytes_bypass (chunk, size_per_thread, loops);
					else
						ReaderSSE2_bypass (chunk, size_per_thread, loops);
				}
				break;

			case AVX:
				if (!random) {
					ReaderAVX (chunk, size_per_thread, loops);
				} else {
#ifdef IS_64BIT
					RandomReaderAVX (chunk_ptrs, p_size_per_thread, loops);
#endif
				}
				break;
			...
			case AVX512:
				if (!random) {
#ifdef IS_64BIT
					ReaderAVX512 (chunk, size_per_thread, loops);
#endif
				}
				break;
#endif
			...
			default:
				if (random) {
					RandomReader (chunk_ptrs, p_size_per_thread, loops);
				} else {
#ifdef x86
					if (size & 128)
						Reader_128bytes (chunk, size_per_thread, loops);
					else
#endif
						Reader (chunk, size_per_thread, loops);
				}
			}
...
//----------------------------------------------------------------------------
long Testing_copy (Testing *self, long *results, unsigned long size, TestingMode mode, bool first_touch)
{
#ifdef x86
	unsigned long long total_count = 0;
	unsigned long diff=0;
	...
	if (first_touch)
	{
		#pragma omp parallel default(none) firstprivate (chunk_src, chunk_dest, size, grain)
		{
			unsigned long threads = omp_get_num_threads();
			unsigned long tid     = omp_get_thread_num();
			...
			if(tid < reminder_grains)
			{
				chunk_src  += (size_per_thread*tid);
				chunk_dest += (size_per_thread*tid);
			}
			else
			{
				chunk_src  += (reminder_grains*grain + size_per_thread*tid);
				chunk_dest += (reminder_grains*grain + size_per_thread*tid);
			}
			memset (chunk_src, 100, size_per_thread);
			memset (chunk_dest, 200, size_per_thread);
		}
	}
	else
	{
	...
	#pragma omp parallel default(none) shared(self, results, mode) firstprivate (chunk_src, chunk_dest, size, grain, usec_per_test) reduction(+:diff, total_count)
	{
		unsigned long long loops;
		...
		while ( (size_per_thread != 0) && (diff < usec_per_test)) {
			total_count += loops;

			if (mode == SSE2)  {
	#ifdef IS_64BIT
				if (size & 128)
					CopySSE_128bytes (chunk_dest, chunk_src, size_per_thread, loops);
				else
					CopySSE (chunk_dest, chunk_src, size_per_thread, loops);
	#else
				CopySSE (chunk_dest, chunk_src, size_per_thread, loops);
	#endif
			}
				...
				if (!(size & 128))
					CopyAVX (chunk_dest, chunk_src, size_per_thread, loops);
			}
...
static long Testing_registerToVectorTest (Testing *self)
{
	...
	$(self, calculate_result, 256, total_count, diff, true);
	$(console, flush);
#endif
	return 0;
}
...
static long Testing_vectorToRegister8 (Testing *self)
{
	...
	if (self->use_sse4) {
		...
		$(self, calculate_result, 64, total_count, diff, true);
		$(console, flush);
	}
#endif
	return 0;
...
static long Testing_vectorToRegister16 (Testing *self)
{
	...
	$(self, calculate_result, 128, total_count, diff, true);
	$(console, flush);
#endif
	return 0;
}
...
static long Testing_memsetTest (Testing *self)
{
	...
	$(self, calculate_result, NT_SIZE, NT_SIZE2, dt, true);
	$(console, flush);
	return 0;
}
...
static long Testing_memcpyTest (Testing *self)
{
	...
	$(self, calculate_result, NT_SIZE, NT_SIZE2, t-t0, true);
	$(console, flush);
	return 0;
}
